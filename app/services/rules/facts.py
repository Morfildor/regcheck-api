...
# Type alias for the internal add() accumulator passed to question-group helpers.
_AddFn = Callable[..., None]
...
# ---------------------------------------------------------------------------
# Question-group helpers — called by _missing_information.
# Each helper is responsible for a coherent concern and calls add() directly.
# ---------------------------------------------------------------------------

def _smart_lock_questions(
    product_type: str | None,
    traits: set[str],
    text: str,
    add: _AddFn,
) -> None:
    """Questions specific to smart-lock products."""
...
def _ev_charging_questions(
    ...
    """Questions specific to EV charging equipment."""
...
def _air_purifier_questions(
    product_type: str | None,
    text: str,
    add: _AddFn,
) -> None:
    """Questions specific to air purifiers."""
...
def _lighting_questions(
    route_family: str | None,
    product_type: str | None,
    text: str,
    add: _AddFn,
) -> None:
    """Questions specific to lighting products."""
...
def _refrigerator_questions(
    product_type: str | None,
    text: str,
    add: _AddFn,
) -> None:
    """Questions specific to refrigerators / freezers."""
...
def _robot_vacuum_questions(
    product_type: str | None,
    text: str,
    add: _AddFn,
) -> None:
    """Questions specific to robot vacuums."""
...
def _life_safety_alarm_questions(
    route_family: str | None,
    product_type: str | None,
    text: str,
    add: _AddFn,
) -> None:
    """Questions specific to smoke / CO alarms and life-safety detectors."""
...
def _machinery_tool_questions(
    ...
    """Questions specific to power tools and machinery."""
    if route_family != "machinery_power_tool" and product_type not in {
        "corded_power_drill", "cordless_power_drill", "industrial_power_tool"
    }:
        return
...
def _toy_questions(
    ...
    """Questions for child-targeted products near the toy boundary."""
...
def _radio_questions(
    traits: set[str],
    matched_products: set[str],
    text: str,
    radio_band_known: bool,
    radio_power_known: bool,
    add: _AddFn,
) -> None:
    """Cross-cutting: radio technology, RF bands, and exposure form-factor."""
    ...
    if (
        not ({"wearable", "handheld", "body_worn_or_applied"} & traits)
        and bool(matched_products & PERSONAL_CARE_PRODUCT_HINTS)
    ):
        add(
            "emf_use_position",
    ...
    if (
        ({"portable", "battery_powered", "cellular"} & traits or bool(matched_products & PERSONAL_CARE_PRODUCT_HINTS))
        and not ({"handheld", "wearable", "body_worn_or_applied"} & traits)
    ):
        add(
...
def _external_psu_question(
    traits: set[str],
    text: str,
    add: _AddFn,
) -> None:
    """Cross-cutting: confirm whether an external PSU or adapter is supplied."""
    if (
        ({"usb_powered", "external_psu"} & traits or "adapter" in text)
        and "external_psu" not in traits
        and not _has_any(text, POWER_EXTERNAL_NEGATION_PATTERNS)
    ):
        add(
            "external_psu",
...
def _battery_questions(
    battery_signal: bool,
    text: str,
    battery_pack_detail_known: bool,
    add: _AddFn,
) -> None:
    """Cross-cutting: battery chemistry, capacity, and pack format."""
...
def _body_contact_questions(
    body_contact_signal: bool,
    text: str,
    add: _AddFn,
) -> None:
    """Cross-cutting: body-contact material and biocompatibility."""
...
def _equipment_questions(
    tool_signal: bool,
    compressor_signal: bool,
    traits: set[str],
    text: str,
    add: _AddFn,
) -> None:
    """Cross-cutting: equipment form-factor, pressure rating, and AV/ICT boundary."""
    def sa(patterns: list[str]) -> bool:
        return any(bool(re.search(p, text)) for p in patterns)
...
def _missing_information(
    ...
    # Pre-computed context signals used across question groups.
    tool_signal = bool(
        matched_products
    ...
    # --- Product-type-specific question groups ---
    ...
    _smart_connectivity_questions(
        smart_signal, traits, text,
        cloud_detail_known, medical_boundary_signal,
        access_detail_known, update_detail_known,
        health_data_signal, data_category_known,
        add,
    )
